function [] = openmp3(path_mp3)
  ...
  if(DFT.Object.CurrentSample > (DFT.Object.TotalSamples - DFT.SampleRate))
    ...
    % Next item
    for items = 1:1:sz(1)
      if(strcmp(DFT.PlayList{items},DFT.Path))
        items = items + 1;
        break;
      end
    end
    ...
    while(~exist(DFT.PlayList{item},'file'))
      item = item + 1;
      if(item > length(DFT.PlayList)) item = 1; end
    ...
    case 'rate', ch_fs  = num2str(DFT.Object.SampleRate);
                 ch_fs  = strcat('Command >> Rate = ',ch_fs,' >>');
                 DFT.UserInput = input(ch_fs,'s');
      switch DFT.UserInput
     ...
     case 'seek'
       ...
       switch DFT.UserInput
         ...
         otherwise
           if(isnan(str2double(DFT.UserInput))),continue; end
           stop(DFT.Object);
    ...
    case 'v', DFT.UserInput=input('Command >> View >>','s');
      switch DFT.UserInput
        case 'acu', selectAlgorithm(DFT.Calc);
        case 'c',
          selectAlgorithm(DFT.Plot2D);
          drawCurvDFT(DFT.Calc.Alg{DFT.Calc.Cur}{1}, DFT.Object, DFT.Signal, DFT.Plot2D.Alg{DFT.Plot2D.Cur});
        case 's',
          selectAlgorithm(DFT.Plot3D);
          drawSurfDFT(DFT.Calc.Alg{DFT.Calc.Cur}{1}, DFT.Object, DFT.Signal, DFT.Plot3D.Alg{DFT.Plot3D.Cur});
        case 'b', continue;
        case 'cmd', display({
                'b - Back';...
                'acl - Select DFT algorithm to be used';...
                '(a)cu - Plot the signal and DFT in 2D mode, prefix to select algorithm';...
                '(a)cu - Plot the signal and DFT in 3D mode, prefix to select algorithm';...
